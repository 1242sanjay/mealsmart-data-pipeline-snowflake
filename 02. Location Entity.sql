use role sysadmin;
use schema mealsmart_db.stage_sch;

list @mealsmart_db.stage_sch.mealsmart_csv_stg;

select $1, $2, $3, $4, $5, $6, $7
from @mealsmart_db.stage_sch.mealsmart_csv_stg/location/delta-load/
...
select 
    ...
    metadata$file_row_number as _row_num,
    metadata$start_scan_time as _start_time,
    current_timestamp as _copy_data_ts
from @mealsmart_db.stage_sch.mealsmart_csv_stg/location/initial-load/ 
(file_format => 'mealsmart_db.stage_sch.csv_file_format') t
;
...
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    from @mealsmart_db.stage_sch.mealsmart_csv_stg/location/initial-load/ t
)
file_format = (format_name = 'stage_sch.csv_file_format')
on_error = abort_statement;

-- check the history using information_schema
select *
...
create or replace table consumption_sch.restaurant_location_dim (
    restaurant_location_hk NUMBER primary key comment 'hash key for the dimension',
    location_id number(38,0) not null comment 'business key',
    ...
    is_union_territory boolean not null default false comment 'union territory flag',
    capital_city_flag boolean not null default false comment 'capital city flag',
    city_tier varchar(6) comment 'city tier',
    zip_code varchar(10) not null comment 'zip code',
    active_flag varchar(10) not null comment 'active flag (indicating current record)',
    eff_start_dt timestamp_tz(9) not null comment 'effective start date for scd2',
    eff_end_dt timestamp_tz(9) comment 'effective end date for scd2',
    current_flag boolean not null default true comment 'indicator of the current record'
)
...
CREATE OR REPLACE PROCEDURE location_data_flow()
