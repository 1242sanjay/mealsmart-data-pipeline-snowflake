...
list @mealsmart_db.stage_sch.mealsmart_csv_stg/restaurant/initial-load/;

select $1, $2, $3, $4, $5, $6, $7
from @mealsmart_db.stage_sch.mealsmart_csv_stg/restaurant/initial-load/restaurant-delhi+NCR.csv
(file_format => 'mealsmart_db.stage_sch.csv_file_format');
...
from (
    select 
     ...
     from @mealsmart_db.stage_sch.mealsmart_csv_stg/restaurant/initial-load/restaurant-delhi+NCR.csv t
)
...
CREATE OR REPLACE TABLE CONSUMPTION_SCH.RESTAURANT_DIM (
    RESTAURANT_HK NUMBER primary key COMMENT 'Hash key for the restaurant location',
    RESTAURANT_ID NUMBER COMMENT 'Restaurant ID without auto-increment',
    NAME STRING(100) COMMENT 'Restaurant name',
    CUISINE_TYPE STRING COMMENT 'Type of cuisine offered',
    PRICING_FOR_TWO NUMBER(10, 2) COMMENT 'Pricing for two people',
    RESTAURANT_PHONE STRING(15) WITH TAG (common.pii_policy_tag = 'SENSITIVE') COMMENT 'Restaurant phone number',
    OPERATING_HOURS STRING(100) COMMENT 'Restaurant operating hours',
    LOCATION_ID_FK NUMBER COMMENT 'Foreign key reference to location',
    ACTIVE_FLAG STRING(10) COMMENT 'Indicates if the restaurant is active',
    OPEN_STATUS STRING(10) COMMENT 'Indicates if the restaurant is currently open',
    LOCALITY STRING(100) COMMENT 'Locality of the restaurant',
    RESTAURANT_ADDRESS STRING COMMENT 'Full address of the restaurant',
    LATITUDE NUMBER(9, 6) COMMENT 'Latitude for the restaurant\'s location',
    LONGITUDE NUMBER(9, 6) COMMENT 'Longitude for the restaurant\'s location',
    EFF_START_DATE TIMESTAMP_TZ COMMENT 'Effective start date for the record',
    EFF_END_DATE TIMESTAMP_TZ COMMENT 'Effective end date for the record (NULL if active)',
    IS_CURRENT BOOLEAN COMMENT 'Indicates whether the record is the current version'
)
