...
list @mealsmart_db.stage_sch.mealsmart_csv_stg/delivery-agent/initial-load;

select $1, $2, $3, $4, $5, $6, $7
from @mealsmart_db.stage_sch.mealsmart_csv_stg/delivery-agent/initial-load/delivery-agent-initial.csv
...
from (
    ...
    from @mealsmart_db.stage_sch.mealsmart_csv_stg/delivery-agent/initial-load t
)
...
CREATE OR REPLACE TABLE consumption_sch.delivery_agent_dim (
    ...
    phone STRING UNIQUE comment 'Phone number, unique',
    vehicle_type STRING comment 'Type of vehicle',
    location_id_fk NUMBER NOT NULL comment 'Location FK (Source System)',                     -- Location ID
    status STRING comment 'Current status of the delivery agent',
    gender STRING comment 'Gender',
    rating NUMBER(4,2) comment 'Rating with one decimal precision',
    eff_start_date TIMESTAMP DEFAULT CURRENT_TIMESTAMP comment 'Effective start date',
    eff_end_date TIMESTAMP comment 'Effective end date (NULL for active record)',
